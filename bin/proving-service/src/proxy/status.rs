...
use crate::{
    commands::worker::ProverTypeSupport, error::ProvingServiceError,
    ...
    async fn handle_request(&self, _session: &mut ServerSession) -> Result<()> {
        ...
        let worker_statuses: Vec<WorkerStatus> = workers
            ...
            .map(|w| WorkerStatus {
                ...
                is_healthy: matches!(w.health_status, WorkerHealthStatus::Healthy),
                retries: w.retries_amount(),
            })
            .collect();

        let status = ProxyStatus {
            version: env!("CARGO_PKG_VERSION").to_string(),
            supported_proof_types: self.load_balancer.supported_proof_types,
