...
impl AccountInterface {
    ...
    pub fn build_send_notes_script(
        ...
        let script = format!(
            "begin\n{}\n{}\n{}\nend",
            self.script_expiration(expiration_delta),
    ...
    /// - the note created by the faucet doesn't contain exactly one asset.
    /// - a faucet tries to distribute an asset with a different faucet ID.
    fn note_creation(&self, output_notes: &[PartialNote]) -> Result<String, AccountInterfaceError> {
    ...
    fn script_expiration(&self, expiration_delta: Option<u16>) -> String {
