use miden_objects::{
    ...
    note::{
        ...
        NoteScript, NoteTag, NoteType, PartialNote,
    },
...
fn test_send_note_script_basic_wallet() {
    ...
    let send_note_script = transaction_script_builder
        .build_send_notes_script(&[partial_note.clone()])
        .unwrap();
...
begin
    ...
    ",
        ...
        asset = word_to_felts_string(&assets.iter().next().unwrap().into())
    );
...
#[test]
fn test_send_script_basic_wallet() {
    let mock_public_key = PublicKey::new([ZERO, ONE, Felt::new(2), Felt::new(3)]);
    let rpo_component = RpoFalcon512::new(mock_public_key);
