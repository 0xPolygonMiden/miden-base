...
```arduino
hash(serial_num, script_hash, input_commitment, vault_hash)
```
...
- To compute the nullifier, one must know all components of the `Note`: serial_num, script_hash, input_commitment, and vault_hash.
