...
- the `Account` including the [AccountID](../accounts.md#account-id) and the [AccountCode](../accounts.md#code) which will be executed during the transaction. 
...
- the `InputNotes` that are being consumed in the transaction (InputNotes), including the corresponding note data, e.g. the [note script](../notes.md#the-note-script) and [serial number](../notes.md#serial-number).
