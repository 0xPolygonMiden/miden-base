...
At its core, a transaction is an executable program - the transaction kernel program - processing the provided inputs and creating the requested outputs. Because the program is being executed in the Miden VM a STARK-proof is being generated for every transaction. 
