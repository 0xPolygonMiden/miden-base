...
In the above, `add_asset` is a kernel procedure `miden::kernels::single_account::account::add_asset` of the Tx Kernel.
...
In the above, `remove_asset` is a kernel procedure `miden::kernels::single_account::account::remove_asset` and `create_note` is a kernel procedure `miden::kernels::single_account::tx::create_note`, both in the Tx Kernel.
