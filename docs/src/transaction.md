...
Compared to most blockchains, where a `Transaction` typically involves more than one account (e.g., sender and receiver), a `Transaction` in Miden involves a single account. To illustrate, Alice (Account 1) sends 3 ETH to Bob. In Miden, sending 3 ETH from Alice to Bob takes two transactions: one in which Alice creates a note containing 3 ETH and one in which Bob consumes that note and receives the 3 ETH. This model removes the need for a global lock on the blockchain's state, enabling Miden to process transactions in parallel.
