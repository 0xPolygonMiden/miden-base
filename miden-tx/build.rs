...
fn extract_kernel_errors(
    errors: &mut BTreeMap<String, (String, String)>,
    file_contents: &str,
) -> Result<()> {
    let regex =
    ...
    for capture in regex.captures_iter(file_contents) {
        let error_name =
            capture.name("name").expect("error name should be captured").as_str().to_owned();
        let error_code =
            capture.name("code").expect("error code should be captured").as_str().to_owned();
        ...
        if let Some((existing_error_code, _)) = errors.get(&error_name) {
        ...
        errors.insert(error_name, (error_code, error_message));
    }
