...
use super::{AccountId, DataStore, DataStoreError, TransactionInputs};
...
mod tests {
    ...
    use miden_mock::{
        ...
        mock::{
            account::{
                ACCOUNT_INCR_NONCE_MAST_ROOT, ACCOUNT_SET_CODE_MAST_ROOT,
    ...
    use miden_objects::{
        accounts::{
            AccountCode, ACCOUNT_ID_FUNGIBLE_FAUCET_ON_CHAIN,
    ...
    use crate::{
        testing::MockDataStore, TransactionExecutor, TransactionHost, TransactionProver,
        TransactionVerifier,
    };
    ...
    fn executed_transaction_account_delta() {
        let data_store =
            MockDataStore::new(AssetPreservationStatus::PreservedWithAccountVaultDelta);
        let mut executor = TransactionExecutor::new(data_store.clone());
        let account_id = data_store.account.id();
        executor.load_account(account_id).unwrap();
        ...
        let new_acct_code =
            AccountCode::new(new_acct_code_ast.clone(), &Assembler::default()).unwrap();
    ...
    fn prove_witness_and_verify() {
        ...
        let proven_transaction =
            ProvenTransaction::read_from_bytes(&serialised_transaction).unwrap();
    ...
    fn test_tx_script() {
        ...
        let tx_script_input_key =
            [Felt::new(9999), Felt::new(8888), Felt::new(9999), Felt::new(8888)];
