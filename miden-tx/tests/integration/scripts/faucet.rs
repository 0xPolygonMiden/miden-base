...
fn prove_faucet_contract_mint_fungible_asset_succeeds() {
    ...
    // --------------------------------------------------------------------------------------------
    let data_store = Arc::new(MockDataStore::with_existing(Some(faucet_account.clone()), Some(vec![])));
...
fn prove_faucet_contract_burn_fungible_asset_succeeds() {
    ...
    // --------------------------------------------------------------------------------------------
    let data_store =
        Arc::new(MockDataStore::with_existing(Some(faucet_account.clone()), Some(vec![note.clone()])));
