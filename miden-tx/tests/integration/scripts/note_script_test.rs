use miden_objects::{
    accounts::{Account, AccountCode, AccountId, AccountStorage, SlotItem, StorageSlot, ACCOUNT_ID_FUNGIBLE_FAUCET_ON_CHAIN, ACCOUNT_ID_REGULAR_ACCOUNT_UPDATABLE_CODE_ON_CHAIN, ACCOUNT_ID_SENDER}, assembly::{ModuleAst, ProgramAst}, assets::{Asset, AssetVault, FungibleAsset}, crypto::rand::{FeltRng, RpoRandomCoin}, notes::{Note, NoteAssets, NoteExecutionMode, NoteInputs, NoteMetadata, NoteRecipient, NoteScript, NoteTag, NoteType}, transaction::TransactionArgs, Felt, NoteError, Word, ONE, ZERO};
...
pub fn get_account_with_custom_account_code(
    ...
    let account_code_src = include_str!("test_account.masm");
...
fn create_note<R: FeltRng>(
    ...
    let note_script = include_str!("test_note_script.masm");
