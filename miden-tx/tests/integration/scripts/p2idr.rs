...
//     // consume the note.
//     //
//     // Case "reclaimable": block height is 4, reclaim block height is 3. Target and sender
// account     // can consume the note. The malicious account should never be able to consume the
// note.     //
// --------------------------------------------------------------------------------------------
...
//     // Check that we got the expected result - TransactionExecutorError and not
// ExecutedTransaction     // Second transaction should not work (sender consumes too early), we
// expect an error     assert!(executed_transaction_2.is_err());
...
//     // Check that we got the expected result - TransactionExecutorError and not
// ExecutedTransaction     // Third transaction should not work (malicious account can never
// consume), we expect an error     assert!(executed_transaction_3.is_err());
...
//     // Check that we got the expected result - TransactionExecutorError and not
// ExecutedTransaction     // Sixth transaction should not work (malicious account can never
// consume), we expect an error     assert!(executed_transaction_6.is_err())
// }
