...
/// block as a reference and creating the account's initial storage and code. Then a random seed is
/// picked and the hash of (SEED, CODE_COMMITMENT, STORAGE_COMMITMENT, EPOCH_BLOCK_HASH) is
/// computed. If the hash's first element has the desired storage mode, account type, version and
/// the high zero bit, the computation part of the ID generation is finished. If not, another random
/// seed is picked and the process repeated.
///
/// The second felt of the ID is the second element of the hash function. Its upper 16 bits are
/// overwritten with the block epoch and the lower 8 bits are zeroed. Thus, the first felt of the ID
/// must derive exactly from the hash, while only part of the second felt is derived from the hash.
///
/// # Design Rationale
///
...
///     - The block epoch is not important beyond the creation process, so placing it in the second
...
///       significant.
/// - The block epoch is placed at the most significant end of the second felt. Its value must be
