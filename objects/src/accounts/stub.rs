use super::{hash_account, AccountId, AccountStorage, Digest, Felt};

// ACCOUNT STUB
// ================================================================================================

/// A stub of an account which contains information that succinctly describes the state of the
/// components of the account that can be extracted from the VM result.  We will remove this in
/// the future when we can extract the full account from the VM result.
///
...
/// - storaget: accounts storage ([AccountStorage]).
...
impl AccountStub {
    ...
    pub fn hash(&self) -> Digest {
        hash_account(self.id, self.nonce, self.vault_root, self.storage.root(), self.code_root)
    }
