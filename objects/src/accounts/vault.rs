...
impl AccountVault {
    ...
    pub fn new(assets: &[Asset]) -> Result<Self, AccountError> {
        Ok(Self {
            asset_tree: TieredSmt::with_leaves(
                assets.iter().map(|asset| (asset.key().into(), (*asset).into())),
            )
    ...
    pub fn has_non_fungible_asset(&self, asset: Asset) -> Result<bool, AccountError> {
        ...
        match self.asset_tree.get_value(asset.key().into()) {
            asset if asset == [ZERO, ZERO, ZERO, ZERO] => Ok(false),
            _ => Ok(true),
        }
    }
    ...
    pub fn get_balance(&self, faucet_id: AccountId) -> Result<u64, AccountError> {
        ...
        match self.asset_tree.get_value([ZERO, ZERO, ZERO, faucet_id.into()].into()) {
            asset if asset == [ZERO, ZERO, ZERO, ZERO] => Ok(0),
    ...
    fn add_fungible_asset(&mut self, asset: FungibleAsset) -> Result<FungibleAsset, AccountError> {
        ...
        let new: FungibleAsset = match self.asset_tree.get_value(asset.key().into()) {
            current if current == [ZERO, ZERO, ZERO, ZERO] => asset,
            current => {
        ...
        self.asset_tree.insert(new.key().into(), new.into());
    ...
    fn add_non_fungible_asset(
        ...
        let old = self.asset_tree.insert(asset.key().into(), asset.into());
    ...
    fn remove_fungible_asset(
        ...
        let mut current: FungibleAsset = match self.asset_tree.get_value(asset.key().into()) {
            current if current == [ZERO, ZERO, ZERO, ZERO] => {
        ...
        self.asset_tree.insert(asset.key().into(), new);
